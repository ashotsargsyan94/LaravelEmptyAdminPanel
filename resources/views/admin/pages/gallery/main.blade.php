        ...
        @if (count($images))
                            ...
                            <div class="gallery-absolute">
                                <div class="gallery-item-actions">
                                    @if(isset($show_home) && $show_home)
                                        <a href="javascript:void(0)" class="gallery-item-action gallery-item-poster" data-id="{{$image->id}}">{!!  ($image->poster)?'<i class="fas fa-eye" data-poster="true"></i>':'<i class="fas fa-eye-slash" data-poster="false"></i>'!!}</a>
                                    @endif
            ...
            @push('js')
                ...
                <script>
                    ...
                    //endregion
                    //regionPoster
                    $('.gallery-item-poster').on('click', function() {

                        var thisItemId = $(this).data('id');
                        var poster = '';
                        var elem=$(this);
                        if( $(this).find('i').data('poster') ){
                            poster=0;
                        }
                        else{
                            poster=1;
                        ...
                        $.ajax({
                            url: '{!! route('admin.gallery.poster') !!}',
                            type: 'post',
                            dataType: 'json',
                            data: {
                                _token: csrf,
                            ...
                            error: function (e) {
                                alert('Внутренная ошибка')
                            },
                            success: function (e) {
                                if(elem.find('i').data('poster')){

                                    elem.html('<i class="fas fa-eye-slash" data-poster="false"></i>')
                                }
                                else{
                                    elem.html('<i class="fas fa-eye" data-poster="true"></i>')
                                }
                            }
                        });
                    });
