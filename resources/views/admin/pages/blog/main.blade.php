...
@section('titleSuffix')| <a href="{!! route('admin.blog.add') !!}" class="text-cyan"><i class="mdi mdi-plus-box"></i> добавить</a>@endsection
@section('content')
    @if(count($items))
        <div class="card">
            <div class="table-responsive">
                <table class="table table-striped m-b-0 columns-middle">
                    ...
                    <tbody  class="table-sortable" data-action="{{ route('admin.blog.sort') }}">
                    @foreach($items as $item)
                        <tr class="item-row" data-id="{!! $item->id !!}">
                            ...
                            @endif
                            <td>
                                <a href="{{ route('admin.blog.edit', ['id'=>$item->id]) }}" {!! tooltip('Редактировать') !!} class="icon-btn edit"></a>
                                <a href="{{ route('admin.gallery', ['gallery'=>'blog_item', 'key'=>$item->id]) }}" {!! tooltip('Галерея') !!} class="icon-btn gallery"></a>
